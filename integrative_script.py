...
class Gene():
    ...
    def __init__(self, gene, locus_tag, sample, genomesFolder):
    ...
    if 'quantile1' in qfile:
        ...
        for i in range(len(genes)):
            ...
            if genes[i] != '-':
                ...
                if added == False:
                    geneObjects[genes[i]] = Gene(genes[i], locuses[i], species, genomesFolder)
            '''
...
with pd.ExcelWriter(f'D:\downloads\Study\laboratory\EloE\data\downloaded\Ralstonia_complete_genome\\10quantiles\\species_cogs.xlsx') as sp_file:
    df_res.to_excel(sp_file)

with pd.ExcelWriter(f'D:\downloads\Study\laboratory\EloE\data\downloaded\Ralstonia_complete_genome\\10quantiles\\group_cogs.xlsx') as sp1_file:
